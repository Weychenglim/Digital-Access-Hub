...
interface GuaranteedCategoryNewsProps {
  category: string
  limit?: number
  showTitle?: boolean
}

export default function GuaranteedCategoryNews({ category, limit = 3, showTitle = true }: GuaranteedCategoryNewsProps) {
  ...
  const fetchNews = async () => {
    try {
      ...
      const apiUrl = new URL("/api/category-news", window.location.origin)
      apiUrl.searchParams.append("category", normalizedCategory)
      apiUrl.searchParams.append("limit", limit.toString())

      console.log(`Fetching guaranteed news for category: ${normalizedCategory}`)
      ...
      setArticles(data) // Set all articles received
    } catch (err) {
  ...
  return (
    <div>
      ...
      {/* For single article display (like in category section) */}
      {limit === 1 && articles.length > 0 ? (
        <a
          href={articles[0].externalUrl}
          target="_blank"
        ...
        >
          <div className="flex flex-col">
            <div className="relative h-48 w-full overflow-hidden">
              <Image
                src={articles[0].imageUrl || "/placeholder.svg?height=200&width=300"}
            ...
            <div className="p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="text-xs text-gray-500">{articles[0].source}</span>
                <span className="text-xs text-gray-500">{articles[0].date}</span>
              </div>
              <h3 className="text-lg font-bold text-gray-800 mb-2 line-clamp-2">{articles[0].title}</h3>
              <p className="text-gray-600 mb-3 text-sm line-clamp-2">{articles[0].description}</p>
        ...
        /* For multiple articles display (grid layout) */
